...
public class Dataset {
    ...
    public ArrayList<Game> getGamesByPrice(int precio) {
        ...
        if (sortedByAttribute.equals("precio")) {
            ...
            System.out.println("Búsqueda binaria tomó " + binaryDuration + " ms.");
        } else {
            ...
            System.out.println("Búsqueda lineal tomó " + linearDuration + " ms.");
        }
        if(result.isEmpty()) {
            ...
            for (Game game : result) {
                System.out.println("Nombre: " + game.getNombre() + ", Categoría: " + game.getCategoria() +
    ...
    public void getGameByPriceRange(int MinPrecio, int MaxPrecio) {
        if (sortedByAttribute.equals("precio")) {
            ...
            for (int i = left; i < data.size() && data.get(i).getPrecio() <= MaxPrecio; i++) {
                System.out.println(data.get(i).getNombre());
                found = true;
            }
            if (!found) {
                System.out.println("No se encontraron juegos en ese rango de precio.");
    ...
    public void getGameByQuality(int calidad) {
        if (sortedByAttribute.equals("calidad")) {
            ...
            while (left <= right) {
                ...
                if (midQuality == calidad) {
                    int target = mid;
                    while (target >= 0 && data.get(target).getCalidad() == calidad) {
                        System.out.println(data.get(target).getNombre());
                        target--;
                    }
                    target = mid + 1;
                    while (target < data.size() && data.get(target).getCalidad() == calidad) {
                        System.out.println(data.get(target).getNombre());
                        target++;
                    }
                    break;
    ...
    void sortByAlgorithm(String algoritmo, String atributo) {
        ...
        switch (algoritmo) {
            case "bubbleSort":
                ...
                System.out.println("Bubble Sort tomó " + measureExecutionTime(startTimeBubble) + " ms.");
                break;
            case "insertionSort":
                long startTimeInsertion = System.nanoTime();
                insertionSort(comparator);
                System.out.println("Insertion Sort tomó " + measureExecutionTime(startTimeInsertion) + " ms.");
                break;
            case "selectionSort":
                long startTimeSelection = System.nanoTime();
                selectionSort(comparator);
                System.out.println("Selection Sort tomó " + measureExecutionTime(startTimeSelection) + " ms.");
                break;
            case "mergeSort":
                long startTimeMerge = System.nanoTime();
                mergeSort(comparator, 0, data.size() - 1);
                System.out.println("Merge Sort tomó " + measureExecutionTime(startTimeMerge) + " ms.");
                break;
            case "quickSort":
                long startTimeQuick = System.nanoTime();
                quickSort(comparator, 0, data.size() - 1);
                System.out.println("Quick Sort tomó " + measureExecutionTime(startTimeQuick) + " ms.");
                break;
            default:
                System.out.println("Algoritmo no reconocido. Se usará Collections.sort().");
                long startTimeCollections = System.nanoTime();
                Collections.sort(data, comparator);
                System.out.println("Collections Sort tomó " + measureExecutionTime(startTimeCollections) + " ms.");
        }
    }
    ...
    static class  LoadData {
        ...
        public static void main(String[] args) {
            ...
            dataset.sortByAlgorithm("bubbleSort","calidad");
            dataset.getGameByPriceRange(10000,19082);
        }
    }
}